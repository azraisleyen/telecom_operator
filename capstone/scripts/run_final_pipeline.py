...
from capstone.src.complaint_signals import add_complaint_signals, export_signal_tables
...
from capstone.src.repeat_complaints import add_repeat_complaint_flags, export_repeat_tables
...
def main():
    ...
    enriched=complaints.merge(profile[['customer_id','lifecycle_status_v2','total_revenue','avg_bill','avg_data_usage','high_value_customer','customer_segment','months_active','active_month_ratio','months_since_last_spending','inactive_gap_count','reactivation_count']],on='customer_id',how='left').merge(bill[bill_cols],on=['customer_id','billing_year_month'],how='left')
    ...
    enriched=add_repeat_complaint_flags(add_complaint_signals(enriched)); enriched=add_friction_v3(enriched); enriched=add_risk_priority(enriched); enriched=add_routing(enriched)
    save_table(enriched,Path(args.data_dir)/'processed/complaints_final_features.parquet')
    export_repeat_tables(enriched,args.output_dir); export_signal_tables(enriched,args.output_dir); export_friction_tables(enriched,args.output_dir); export_risk_tables(enriched,args.output_dir)
